...
foreach($wallets as $wallet)
{
    ...
    <div class="card mb-4">
        <div class="card-header d-flex alert alert-info pb-0">
            <a class="my-1 py-1 mr-auto" href="../public/index.php?route=editWallet&walletId=<?= $wallet->getId(); ?>"><h4 ><?= $wallet->getTitle(); ?></h4></a>
            <a class="mr-3 mb-2 p-2" href="../public/index.php?route=editWallet&walletId=<?= $wallet->getId(); ?>">
            ...
            <a class="mr-1 mb-2 p-2 text-danger" href="../public/index.php?route=deleteWallet&walletId=<?= $wallet->getId(); ?>">
    ...
    <?php
    $sum = 0;
    foreach($wallet->getWalletHasCoins() as $coin)
    {
        $sum += $coin->getCoinPrice() * $coin->getCoinQuantity();
        ?>
        <ul class="list-group">
            <li class=" ml-2 mr-2 list-group-item border-bottom d-flex">
                <span><strong class="text-success"><?= $coin->getCoinSymbol(); ?></strong> &#8771 <?= $coin->getCoinPrice(); ?> &euro; x <?= $coin->getCoinQuantity(); ?></span>
                <div class="ml-auto">
                    <a class="text-danger" href="../public/index.php?route=deleteCoinFromWallet&walletId=<?= $wallet->getId(); ?>coinId=<?= $coin->getCoinId(); ?>">
